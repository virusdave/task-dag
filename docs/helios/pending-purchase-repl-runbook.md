...
5. Click **Accept candidate** only after the candidate row review looks right.
   ...
   current/applyable.
6. Queue apply only from the current revision. The server rejects candidate,
   superseded, failed, or otherwise non-current packets even if the UI is stale.
7. If the accepted revision is wrong, open the older revision and use
...
journalctl -u helios-worker.service -n 80 --no-pager | rg 'pending_purchases.refine|refinement'
curl -sSI https://helios.freshlybaked.us/healthzz
```

Then smoke the UI with an operator session:

- Open a current packet; the refinement panel should show the current revision
  and root version.
- Submit low-risk test feedback only against a non-critical packet.
