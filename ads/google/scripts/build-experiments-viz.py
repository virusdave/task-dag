...
def _is_broken(row: dict) -> bool:
    ...
    the account of impressions and signal account-quality problems."""
    return (
        row.get("ad_status") == "enabled"
        and row.get("policy_status") in ("disapproved", "approved_limited")
    )
...
def _find_clone_source(broken_group: str, snapshot: list[dict]) -> tuple[dict | None, str]:
    """Pick the best surviving ad we can clone into the broken group.
    Order: same group (sibling) > same family_tags (family-mate). Returns
    (source_row, 'sibling'|'family'|'none')."""
    broken_rows = [r for r in snapshot if r.get("ad_group_name") == broken_group]
    if not broken_rows:
        return None, "none"
    # 1. Sibling within the same ad group
    siblings = sorted(
        (r for r in broken_rows if _is_healthy(r)),
        key=_ad_priority,
        reverse=True,
    )
    if siblings:
        return siblings[0], "sibling"
    # 2. Family-mate: matching creative_theme + product_tag + geo_target
    ft = broken_rows[0].get("family_tags") or {}
    if ft:
        family_mates = []
        for r in snapshot:
            if r.get("ad_group_name") == broken_group:
                continue
            other_ft = r.get("family_tags") or {}
            if (
                ft.get("creative_theme") == other_ft.get("creative_theme")
                and ft.get("product_tag") == other_ft.get("product_tag")
                and _is_healthy(r)
            ):
                family_mates.append(r)
        family_mates.sort(key=_ad_priority, reverse=True)
        if family_mates:
            return family_mates[0], "family"
    return None, "none"


def build_recovery(snapshot: list[dict]) -> tuple[str, str, dict]:
    """Produce a recovery CSV + RECOVERY.md plus stats.

    The CSV creates one new Enabled RSA per broken ad group, cloned from
    the best surviving source we can find. The Campaign column is left
    blank because the snapshot doesn't record campaign_name; the human
    must do a quick find/replace in Editor (or fill in before import)
    to point the rows at the right existing campaign. RECOVERY.md spells
    this out and also provides a paste-able bulk-pause list for the UI.
    """
    from collections import defaultdict
    groups: dict[str, list[dict]] = defaultdict(list)
    for r in snapshot:
        groups[r.get("ad_group_name") or ""].append(r)

    broken_groups = []
    for g, ads in groups.items():
        bad = [a for a in ads if _is_broken(a)]
        if bad:
            broken_groups.append((g, bad))
    broken_groups.sort(key=lambda x: -len(x[1]))
    ...
    md_lines.append("# Recovery plan -- repair limited/disapproved assets\n")
    md_lines.append("Generated by ads/google/scripts/build-experiments-viz.py.\n")
    ...
    for g, bad in broken_groups:
        for ad in bad:
            first_h = (ad.get("headlines") or [""])[0]
            md_lines.append(
                f"| {g} | {ad.get('ad_id','')} | "
                f"{ad.get('policy_status','')}/{ad.get('serving_status','')} | "
                f"{first_h} |"
            )
            pause_lines.append(f"{g}\t{ad.get('ad_id','')}")
            n_pause += 1

    md_lines.append("\n## Step 2 -- import `002-recovery.csv`\n")
    ...
    for g, bad in broken_groups:
        ...
        for i, x in enumerate(h):
            row[f"Headline {i + 1}"] = x
        for i, x in enumerate(d):
            row[f"Description {i + 1}"] = x
        for i, p in enumerate((src_paths or [])[:2]):
            row[f"Path {i + 1}"] = p
        csv_rows.append(row)
        n_cloned += 1
        md_lines.append(f"- **{g}** <- cloned from {kind} `{src.get('ad_id')}`")

    if no_source_groups:
        md_lines.append("\n## Groups needing fresh creative (no clone source available)\n")
...
<main>
  ...
  <section>
    <h2>🚑 Recovery — limited/disapproved enabled ads <span class="count">({recovery_pause_count})</span></h2>
    <div class="cards">
...
def build_html(out_path: Path) -> Path:
    ...
    page = PAGE_TEMPLATE.format(
        ...
        recovery_broken_groups=csv_stats.get("recovery", {}).get("broken_groups", 0),
