...
command -v shellcheck >/dev/null 2>&1 && {
    ...
    shellcheck -S error "$TD" "$(dirname "$TD")/task-dag.d/cross-repo.sh" \
        ...
        "$here/ci-repair-audit.sh" "$here/ci-repair-projections.sh" "$here/ci-repair-evidence.sh" \
