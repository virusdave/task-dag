...
command -v shellcheck >/dev/null 2>&1 && {
    ...
    shellcheck -S error "$TD" "$(dirname "$TD")/task-dag.d/cross-repo.sh" \
        ...
        "$here/ci-repair-ticket.sh" "$here/ci-tree-fix-outcome.sh" || exit 1
}
echo "== bash -n =="
bash -n "$TD" \
...
rc=0
for t in complete-safety.sh ingest-loop.sh ingest-selfheal.sh blocked-overlay.sh transitive-block.sh claim-pid.sh claim-force-steal.sh breakdown-self-claim.sh tree-fix-trailers.sh ci-chain-cas.sh ci-classifier.sh ci-verify-target.sh ci-repair-ticket.sh ci-tree-fix-outcome.sh; do
    echo "== $t =="
    bash "$here/$t" "$TD" || rc=1
done
exit "$rc"
