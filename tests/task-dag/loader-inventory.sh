...
expected=(source-contract.sh json.sh cas-retry.sh git-objects.sh task-model.sh epic-registry.sh child-map.sh claim-model.sh epic-create.sh epic-compose.sh ref-schema.sh repository-identity.sh github-origin.sh blocked-core.sh activation-fleet.sh activation.sh ci-chains.sh ci-repair.sh comment-watchdog.sh edges.sh facts.sh reconciliation-core.sh semantic-consumer.sh status-projection.sh scheduling-fence.sh cross-repo.sh edges-write.sh edges-prune.sh graph-converge.sh legacy-edges.sh mailbox.sh materialise-parsing.sh materialise-census-capture.sh materialise-intent.sh materialise-producer.sh materialise-reconcile.sh materialise.sh reconcile.sh semantic-migration.sh root-containment.sh retire-owned.sh)
if [ "${direct[*]}" = "${expected[*]}" ] \
