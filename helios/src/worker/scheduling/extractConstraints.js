import { DEFAULT_UNSPECIFIED_WEEKLY_MAX_HOURS, DEFAULT_UNSPECIFIED_WEEKLY_PREFERRED_HOURS, DEFAULT_SCHEDULING_WEEK_DEFINITION, LLMExtractedConstraintsSchema, NormalizedSolverInputSchema, SchedulingAvailabilityWindowSchema, SchedulingWeekWindowSchema, applySchedulingWeeklyHoursPolicy, } from '../../shared/contracts/index.js';
...
function singletonFriendlyArray(itemSchema) {
...
export async function extractSchedulingConstraints(input) {
    ...
    let availabilitySupplementPromise = null;
...
function normalizeExtractedConstraints(extractedConstraints) {
    const issues = [...extractedConstraints.issues];
    ...
    const normalizedEmployees = extractedConstraints.employees.map((employee) => {
        ...
        return {
            ...
            qualifications: [...(employeeQualificationsByName.get(normalizeEmployeeConstraintName(employee.name)) ?? new Set())].sort(),
        };
    });
...
async function requestMantleStructuredObject(input) {
...
function coerceSchedulingExtractionObject(value, input, employeeAvailabilityOverrides = null) {
...
function buildUniqueId(baseId, counts) {
    const nextCount = (counts.get(baseId) ?? 0) + 1;
    counts.set(baseId, nextCount);
    return nextCount === 1 ? baseId : `${baseId}-${nextCount}`;
}
function coerceSchedulingEmployees(value, qualificationAssignments, employeeAvailabilityOverrides) {
...
async function extractEmployeeAvailabilitySupplements(input) {
    ...
    return new Map(parsed.employees
        .filter((employee) => employee.availability.length > 0)
        .map((employee) => [normalizeEmployeeConstraintName(employee.name), employee.availability]));
}
...
function coerceSchedulingAvailabilitySupplementObject(value) {
    return {
        ...value,
...
function buildSchedulingSourceSections(sourceText) {
    return {
        availabilitySection: extractSourceSection(sourceText, /The availability for the staff is:\s*([\s\S]*?)(?:\n\nThe preferences (?:of|for) the staff are:|$)/i),
...
function extractSourceSection(sourceText, pattern) {
    return sourceText.match(pattern)?.[1]?.trim() ?? '';
}
function shouldUseParagraphAvailabilitySupplement(value, availabilitySection) {
    ...
    return value.some((employee) => {
        ...
        return employee.availability.some((window) => (!isRecord(window)
...
function extractEmployeeNamesFromParsedValue(value) {
    if (!Array.isArray(value)) {
        return [];
    }
    return value
...
function coerceShiftRequirements(value, sourceDerivedShiftRequirements) {
    ...
    const existingKeys = new Set(normalizedRequirements
        .filter(isRecord)
    ...
    const missingRequirements = sourceDerivedShiftRequirements.filter((shiftRequirement) => !existingKeys.has(buildShiftRequirementKey(shiftRequirement)));
    return missingRequirements.length > 0 ? [...normalizedRequirements, ...missingRequirements] : normalizedRequirements;
}
function coerceAvailabilityWindows(value) {
    if (!Array.isArray(value)) {
        return value;
    }
...
function coerceWeekdayArray(value) {
    if (!Array.isArray(value)) {
        return value;
    }
    return value
...
function buildEmployeeQualificationAssignments(sourceText) {
    const assignments = new Map();
    const employeeSection = extractEmployeeSection(sourceText);
    if (!employeeSection) {
        return assignments;
    }
    addQualificationAssignmentsFromMatches(assignments, employeeSection, /The following are\s+([^:\n]+):\s*([^\.]+)\./gi, 1, 2);
    addQualificationAssignmentsFromMatches(assignments, employeeSection, /All remaining employees are\s+([^\.]+)\.\s*They are:\s*([^\.]+)\./gi, 1, 2);
    addQualificationAssignmentsFromMatches(assignments, employeeSection, /([A-Z][a-z]+(?:\s+and\s+[A-Z][a-z]+)+)\s+are\s+([^\.]+)\./g, 2, 1);
    addQualificationAssignmentsFromMatches(assignments, employeeSection, /\b([A-Z][a-z]+)\s+is also\s+([^\.]+?)(?:\.|,)/g, 2, 1);
    addQualificationAssignmentsFromMatches(assignments, employeeSection, /\b([A-Z][a-z]+)\s+is\s+([^\.]+?)(?:\.|,)/g, 2, 1);
    return assignments;
}
function extractEmployeeSection(sourceText) {
...
function addQualificationAssignmentsFromMatches(assignments, sourceText, pattern, qualificationsIndex, namesIndex) {
    for (const match of sourceText.matchAll(pattern)) {
        ...
        for (const name of parseNameList(match[namesIndex] ?? '')) {
            ...
            const assignment = assignments.get(normalizedName) ?? new Set();
...
function extractQualificationsFromPhrase(phrase) {
...
function buildShiftRequirementsFromSource(sourceText) {
    ...
    let activeDays = [];
    let pendingTaggedRequirementStartIndex = null;
    for (const line of lines) {
        ...
        if (parsedShiftTagNote.length > 0
            && pendingTaggedRequirementStartIndex !== null
            && pendingTaggedRequirementStartIndex < shiftRequirements.length) {
            appendTagsToShiftRequirements(shiftRequirements, pendingTaggedRequirementStartIndex, parsedShiftTagNote);
            pendingTaggedRequirementStartIndex = null;
            continue;
        }
        if (/:$/.test(line)) {
...
function parseDayRangeHeader(line) {
    const match = line.match(/^(sunday|monday|tuesday|wednesday|thursday|friday|saturday)\s+through\s+(sunday|monday|tuesday|wednesday|thursday|friday|saturday)(?:\s*,\s*[^:]+)?(?:\s+there are)?:\s*$/i);
    if (!match) {
        return [];
    }
...
function parseShiftTagNoteLine(line) {
    const normalizedLine = line.replace(/[“”]/g, '"');
    const match = normalizedLine.match(/^(?:the above shifts|these shifts|those shifts)\s+are\s+all\s+tagged\s+(?:as\s+)?"?([^".]+?)"?\s+shifts?\.?$/i);
    if (!match) {
        return [];
    }
    return match[1]
...
function appendTagsToShiftRequirements(shiftRequirements, startIndex, tags) {
...
function parseSourceShiftLine(line) {
...
function parseSourceTimeRanges(timesSegment) {
    const parsedRanges = [];
...
function pickBestTimeRange(rawStart, rawEnd, previousEndMinute) {
...
function buildClockCandidates(value) {
...
function normalizeTimeOfDayValue(value) {
    if (typeof value !== 'string') {
        return value;
    }
...
function isNormalizedTimeOfDayValue(value) {
    return typeof value === 'string' && /^([01]\d|2[0-3]):[0-5]\d$/.test(value);
}
function normalizeAvailabilityTimeOfDayValue(value, boundary) {
    if (typeof value !== 'string') {
        return value;
    }
...
function buildShiftRequirementKey(input) {
...
function buildEmployeeIdByName(employees, issues) {
...
function buildEmployeeQualificationsByName(employees) {
    const qualificationsByName = new Map();
    for (const employee of employees) {
        qualificationsByName.set(normalizeEmployeeConstraintName(employee.name), new Set(employee.qualifications.map(normalizeQualification)));
    }
    return qualificationsByName;
}
function buildKnownQualifications(employees) {
    const knownQualifications = new Set();
    for (const employee of employees) {
...
function buildRequiredQualifications(shiftRequirement, knownQualifications) {
...
function deriveRoleQualification(shiftRequirement) {
    if (shiftRequirement.roleLabel === null
...
function deriveKnownQualificationsFromShift(shiftRequirement, knownQualifications) {
...
function shiftReferencesQualification(shiftRequirement, qualification) {
...
function inferImplicitContinuityGroups(shiftRequirements, knownQualifications) {
...
function isPassiveQualifiedShift(shiftRequirement, requiredQualifications) {
...
function shiftsTouchOrOverlap(left, right) {
...
function compareContinuityTarget(source, left, right) {
...
function buildContinuityTargetScore(source, target) {
...
function normalizeEmployeeConstraintName(value) {
...
function describeStructuredOutputError(error) {
...
function normalizeJsonEnvelope(content) {
...
function parseJsonObjectStrict(content) {
...
function repairMalformedJson(content) {
    let repaired = '';
    let inString = false;
    let isEscaped = false;
...
export { SCHEDULING_EXTRACTION_MODEL, SCHEDULING_EXTRACTION_PROMPT_VERSION, };
...
//# sourceMappingURL=extractConstraints.js.map