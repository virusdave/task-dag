...
export async function runCatalogPendingPurchasesGenerateJob(
  ...
  for (const group of liveCollection.groups.values()) {
    ...
    processedGroups += 1
    if (processedGroups === 1 || processedGroups === totalGroups || processedGroups % 5 === 0) {
      await updateJobProgress(context.id, {
        completed: processedGroups,
        message: `Resolved ${processedGroups} of ${totalGroups} pending distributor product${totalGroups === 1 ? '' : 's'}. Current item: ${group.distributorProductName}.`,
        phase: 'Resolving catalog actions',
        phaseCount: 3,
        phaseIndex: 2,
        total: totalGroups > 0 ? totalGroups : null,
