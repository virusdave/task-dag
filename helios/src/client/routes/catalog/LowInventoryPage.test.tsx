...
describe('LowInventoryView', () => {
  it('renders location-first groups, categories, the default cannabis filter, and every package read-only', () => {
    ...
    expect(html).toContain('Flower / Indoor')
    expect(html).toContain('Cannabis products only')
    expect(html).toContain('src="https://cdn.example.test/product.jpg"')
    expect(html).toContain('aria-label="A deliberately long product name that must wrap cleanly on a narrow phone screen; expand image"')
