...
# Materialise-Child-Epic close barrier and durable intent projection.
#
...
for prerequisite in remote_ref_sha_checked taskdag_materialise_groups_json_from_message \
    taskdag_materialise_parent_number; do
    if ! declare -F "$prerequisite" >/dev/null; then
        echo "Error: materialise-intent.sh requires provider $prerequisite to be loaded first" >&2
