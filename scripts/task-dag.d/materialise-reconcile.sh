...
_taskdag_materialise_record_with_prior_identity() { # prior; reads successor JSON on stdin
    local prior=$1
    jq -cS --argjson prior "$prior" '
      . + (if $prior.schema==3 then {
        schema:3,slotId:$prior.authoritySlotId,declaredSlotId:$prior.declaredSlotId,
        authoritySlotId:$prior.authoritySlotId,createCapable:$prior.createCapable
      } else {schema:1,slotId:$prior.slotId} end)'
}
...
_taskdag_materialise_recover() { # tip token producer slot uncertain declaration body
    ...
    record=$(jq -ncS --argjson prior "$state" --argjson issue "$issue" --argjson provider "$issues" --arg actor "${TASK_DAG_CLAIMER:-materialisation-reconciler}" --arg now "$now" --arg tip "$tip" --arg authorityTip "$(jq -r .authorityTip <<<"$token")" --arg priorDigest "$prior_digest" --arg creator "$(jq -r .appCreatorNodeId <<<"$producer")" --arg bodySha256 "$(sha256sum "$body" | awk '{print $1}')" --arg title "$(jq -r .title <<<"$declaration")" '{state:"issue-adopted",declarationDigest:$prior.declarationDigest,operationId:$prior.operationId,generation:($prior.generation+1),fence:($prior.generation+2),activation:$prior.activation,actor:$actor,authoritativeTimestamp:$now,predecessorStateDigest:$priorDigest,originReadback:{activationAuthorityTip:$authorityTip,materialisationTip:$tip},createAttemptId:$prior.createAttemptId,adoptedIssue:{repositoryId:$provider.repositoryId,issueNodeId:$issue.node_id,number:$issue.number},providerReceipt:{repositoryId:$provider.repositoryId,creatorNodeId:$creator,observedAt:$provider.observedAt,paginationQuery:$provider.paginationQuery,pagesFetched:$provider.pagesFetched,exhausted:true,matchCount:1,matchedIdentity:{issueNodeId:$issue.node_id,number:$issue.number,createdAt:$issue.created_at,title:$title,bodySha256:$bodySha256,operationId:$prior.operationId,declarationDigest:$prior.declarationDigest}}}' | _taskdag_materialise_record_with_prior_identity "$state") || return 3
...
_taskdag_materialise_finalize() { # tip token slot adopted declaration
    ...
    if [ "$(jq -r .state <<<"$adopted")" = issue-adopted ]; then
        ...
        pending=$(jq -ncS --argjson prior "$adopted" --argjson activation "$activation" --arg actor "${TASK_DAG_CLAIMER:-materialisation-reconciler}" --arg now "$now" --arg tip "$tip" --arg authorityTip "$(jq -r .authorityTip <<<"$token")" --arg priorDigest "$prior_digest" --arg markerRef "$marker_ref" --arg markerCommit "$marker_commit" '{state:"marker-durable-delegation-pending",declarationDigest:$prior.declarationDigest,operationId:$prior.operationId,generation:($prior.generation+1),fence:($prior.generation+2),activation:$activation,actor:$actor,authoritativeTimestamp:$now,predecessorStateDigest:$priorDigest,originReadback:{activationAuthorityTip:$authorityTip,materialisationTip:$tip},adoptedIssue:$prior.adoptedIssue,markerRef:$markerRef,markerCommit:$markerCommit}' | _taskdag_materialise_record_with_prior_identity "$adopted") || return 3
    ...
    now=$(date -u +%Y-%m-%dT%H:%M:%SZ); prior_digest=$(printf '%s\n' "$adopted" | sha256sum | awk '{print $1}')
    final=$(jq -ncS --argjson prior "$adopted" --arg actor "${TASK_DAG_CLAIMER:-materialisation-reconciler}" --arg now "$now" --arg tip "$tip" --arg authorityTip "$(jq -r .authorityTip <<<"$token")" --arg priorDigest "$prior_digest" --arg delegationRef "$delegation" --arg edgeId "$edge" '{state:"final",declarationDigest:$prior.declarationDigest,operationId:$prior.operationId,generation:($prior.generation+1),fence:($prior.generation+2),activation:$prior.activation,actor:$actor,authoritativeTimestamp:$now,predecessorStateDigest:$priorDigest,originReadback:{activationAuthorityTip:$authorityTip,materialisationTip:$tip},adoptedIssue:$prior.adoptedIssue,markerRef:$prior.markerRef,markerCommit:$prior.markerCommit,delegationRef:$delegationRef,edgeId:$edgeId}' | _taskdag_materialise_record_with_prior_identity "$adopted") || return 3
...
_taskdag_materialise_reconcile_slot() { # slot
    ...
    case "$(jq -r .state <<<"$state")" in
      ...
      batch-reserved-before-create)
        ...
        prior_digest=$(printf '%s\n' "$state" | sha256sum | awk '{print $1}')
        record=$(jq -ncS --argjson prior "$state" --arg actor "${TASK_DAG_CLAIMER:-materialisation-reconciler}" --arg now "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg tip "$tip" --arg authorityTip "$(jq -r .authorityTip <<<"$token")" --arg priorDigest "$prior_digest" --arg attempt "$attempt" --arg producerDigest "$(printf '%s\n' "$producer" | sha256sum | awk '{print $1}')" --arg repository "$(jq -r .peerRepo.name <<<"$declaration")" --arg repositoryId "$provider_id" --arg timeFloor "$provider_time" '{state:"create-in-flight-or-uncertain",declarationDigest:$prior.declarationDigest,operationId:$prior.operationId,generation:($prior.generation+1),fence:($prior.generation+2),activation:$prior.activation,actor:$actor,authoritativeTimestamp:$now,predecessorStateDigest:$priorDigest,originReadback:{activationAuthorityTip:$authorityTip,materialisationTip:$tip},createAttemptId:$attempt,producerRecordDigest:$producerDigest,provider:{repository:$repository,repositoryId:$repositoryId,timeFloor:$timeFloor}}' | _taskdag_materialise_record_with_prior_identity "$state") || { rm -rf "$tmp"; return 3; }
        _taskdag_materialise_append_state "$tip" "$token" "$slot" "$record"; rc=$?
        if [ "$rc" -eq 0 ]; then
      ...
      create-in-flight-or-uncertain)
        if [ "$(jq -r .state <<<"$state")" = create-in-flight-or-uncertain ]; then
            ...
            if git cat-file -e "$tip:$authorization" 2>/dev/null; then
                ...
                attempt=$(od -An -N32 -tx1 /dev/urandom | tr -d ' \n') || { rm -rf "$tmp"; return 3; }
                prior_digest=$(printf '%s\n' "$state" | sha256sum | awk '{print $1}')
                record=$(jq -ncS --argjson prior "$state" --argjson activation "$(jq -c '{epoch,digest,guardVersion}' <<<"$token")" --arg actor "${TASK_DAG_CLAIMER:-materialisation-reconciler}" --arg now "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg tip "$tip" --arg authorityTip "$(jq -r .authorityTip <<<"$token")" --arg priorDigest "$prior_digest" --arg attempt "$attempt" --arg producerDigest "$(printf '%s\n' "$producer" | sha256sum | awk '{print $1}')" --arg repository "$(jq -r .peerRepo.name <<<"$declaration")" --arg repositoryId "$provider_id" --arg timeFloor "$provider_time" --arg authorizationDigest "$authorization_digest" '{state:"create-in-flight-or-uncertain",declarationDigest:$prior.declarationDigest,operationId:$prior.operationId,generation:($prior.generation+1),fence:($prior.generation+2),activation:$activation,actor:$actor,authoritativeTimestamp:$now,predecessorStateDigest:$priorDigest,originReadback:{activationAuthorityTip:$authorityTip,materialisationTip:$tip},createAttemptId:$attempt,producerRecordDigest:$producerDigest,provider:{repository:$repository,repositoryId:$repositoryId,timeFloor:$timeFloor},rearmAuthorizationDigest:$authorizationDigest}' | _taskdag_materialise_record_with_prior_identity "$state") || { rm -rf "$tmp"; return 3; }
                _taskdag_materialise_append_state "$tip" "$token" "$slot" "$record"; rc=$?
                if [ "$rc" -eq 0 ]; then
                    taskdag_materialise_provider_create "$(jq -r .peerRepo.name <<<"$declaration")" "$(jq -r .title <<<"$declaration")" "$body" >/dev/null 2>&1 || :
                elif [ "$rc" -ne 10 ]; then rm -rf "$tmp"; return 3
...
cmd_materialise_reconcile() {
    ...
    if [ -n "$slot" ]; then
        authority=$(_taskdag_materialise_fetch_authority) || return $?
        IFS=$'\t' read -r tip token producer <<<"$authority"
        slot=$(_taskdag_materialise_resolve_cli_slot "$tip" "$slot") || { echo "Error: --slot must name one unambiguous authority slot" >&2; return 3; }
        _taskdag_materialise_reconcile_slot "$slot"; return $?
    fi
    authority=$(_taskdag_materialise_fetch_authority) || return $?
    IFS=$'\t' read -r tip token producer <<<"$authority"
