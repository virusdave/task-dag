...
cmd_close_epic() {
    ...
    # Idempotency: epic SHA already a non-primary parent on master?
    if git log master --format='%P' 2>/dev/null | grep -qw "$epic_sha"; then
        _xrepo_log "close-epic: epic ${epic_sha} already closed on master"
        return 0
    fi
